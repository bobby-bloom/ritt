...
class AppKeys {
  ...
  static final timeEntries = StoreKey<List<TimeEntryResponse>>(
    'time-entries',
    fromJson: (json) =>
        (json as List).map((x) => TimeEntryResponse.fromJson(x)).toList(),
    toJson: (entries) => entries.map((x) => x.toJson()).toList(),
  );
